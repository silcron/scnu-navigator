...
function intentGroup(service) {
  return service?.canonical_id ||
    service?.intent_group ||
    service?.id ||
    '';
}
...
function canonicalRepresentatives(rows = services) {
  ...
  for (const s of rows) {
    ...
    if (
      !prev ||
      representativePriority(s) < representativePriority(prev)
    ) {
      byGroup.set(group, s);
    }
  }
...
function shortlist(query, max = 60) {
  ...
  try {
    ...
    if (local?.status === 'answer') {
      for (const item of (local.items || [])) {
        if (
          item?.service?.id &&
          !seedIds.includes(item.service.id)
        ) {
          seedIds.push(item.service.id);
        }
      }
    }
  } catch (_) {}
  ...
  const scored = services.map(s => {
    ...
    let score = seedIds.includes(s.id)
      ? 10000 - seedIds.indexOf(s.id)
      : 0;
    ...
    for (const f of fields) {
      ...
      if (qn === fn) {
        score += 100;
        evidence.push(String(f));
      } else if (
        qn.includes(fn) &&
        fn.length >= 3
      ) {
        score += 35 + Math.min(20, fn.length);
        evidence.push(String(f));
      } else if (
        fn.includes(qn) &&
        qn.length >= 3
  ...
  .sort(
    (a, b) =>
      b.score - a.score ||
      representativePriority(a.s) - representativePriority(b.s) ||
  ...
  // 기본 검색이 정말 아무 신호도 못 잡은 경우에는
  // 랜덤 후보 60개를 주지 않고 canonical 전체 대표를 Gemini에 제공.
...
function sanitize(
  ...
  const candidateIds =
    new Set(candidates.map(x => x.s.id));
  ...
  const maxIds =
    Math.max(
      ...
      Math.min(
        5,
        Number(options.max_ids) || 5
      )
    );

  const intents =
    Array.isArray(result?.intents)
      ? result.intents.slice(0, 5)
      : [];
  ...
  if (intents.length) {
    for (const it of intents) {
      ...
      const id =
        String(it?.service_id || '');

      const span =
        String(it?.evidence_span || '')
          .trim()
          .slice(0, 120);
    ...
    // 이전 controlled test 응답과 임시 호환.
    const raw =
      Array.isArray(result?.service_ids)
        ? result.service_ids
        : [];
  ...
  return {
    ...
    confidence:
      ['high', 'medium', 'low']
        ...
        : 'low',
    needs_clarification:
      Boolean(result?.needs_clarification) ||
      statuses.some(
...
module.exports = async function handler(req, res) {
  ...
  for (
    ...
    try {
      ...
      // timeout / 일시적 fetch network error는
      // 한 번만 재시도.
      if (
        attempt < 2 &&
        (
  ...
  if (!upstream?.ok) {
    ...
    // 실제 production Gemini 검증 중 임시 진단정보.
    // API key는 포함하지 않는다.
    return res
      .status(200)
      .json({
